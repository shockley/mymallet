...
/**
 * Built upon cc.mallet.topics.ParallelTopicModel
 * Attempting to add the 'fold-in' facility
 * (i.e., annotate new documents according to old topic-word distrib)
 * 
 * @author David Mimno, Andrew McCallum
 */

public class NewParallelTopicModel implements Serializable {

	public static final int UNASSIGNED_TOPIC = -1;

	public static Logger logger = MalletLogger.getLogger(NewParallelTopicModel.class.getName());
	...
	/**
	 * histogram of document sizes, for dirichlet estimation
	 */
	public int[] docLengthCounts; // 
	/**
	 * histogram of document/topic counts, 
	 * indexed by (topic index, sequence position index)
	 */
	...
	public NewParallelTopicModel (int numberOfTopics) {
		this (numberOfTopics, numberOfTopics, DEFAULT_BETA);
	}
	
	public NewParallelTopicModel (int numberOfTopics, double alphaSum, double beta) {
	...
	public NewParallelTopicModel (LabelAlphabet topicAlphabet, double alphaSum, double beta)
	{
	...
	/**
	 * to support incremental esitimation,
	 * the assumption is that new docs doon't have new words
	 * @author Shockley Xiang Li
	 * @param newlist
	 */
	public void addNewInstances (InstanceList newlist) {
		...
		for (Instance instance : newlist) {
			...
			for (int position = 0; position < tokens.getLength(); position++) {
				int type = alphabet.lookupIndex(tokens.getObjectAtPosition(position));
				typeTotals[ type ]++;
			}
		}
		...
		//  the type occurs.
		int [][] newTypeTopicCounts = new int [numTypes][];
		for (int type = 0; type < numTypes; type++) {
			if (typeTotals[type] > maxTypeCount) { maxTypeCount = typeTotals[type]; }
			newTypeTopicCounts[type] = new int[ Math.min(numTopics, typeTotals[type]) ];
		}
		
		for(int type = 0; type < numTypes; type++){
			for(int i = 0;i < typeTopicCounts[type].length;i++)
				newTypeTopicCounts[type][i] = typeTopicCounts[type][i];
			//fill the rest with 0
			for(int i = typeTopicCounts[type].length;i < newTypeTopicCounts[type].length;i++)
				newTypeTopicCounts[type][i] = 0;
		}
		...
		for (Instance instance : newlist) {
			...
			for (int position = 0; position < topics.length; position++) {
				...
				int type = alphabet.lookupIndex(tokens.getObjectAtPosition(position));
				int[] currentTypeTopicCounts = newTypeTopicCounts[ type ];
				...
				//Shockley:
				int index = typeTopicCounts[type].length;
	...
	public static NewParallelTopicModel read (File f) throws Exception {

		NewParallelTopicModel topicModel = null;

		ObjectInputStream ois = new ObjectInputStream (new FileInputStream(f));
		topicModel = (NewParallelTopicModel) ois.readObject();
		ois.close();
	...
	public static void main (String[] args) {
		...
		try {
			...
			NewParallelTopicModel lda = new NewParallelTopicModel (numTopics, 50.0, 0.01);
