...
public class WorkerRunnable implements Runnable {
	...
	public void buildLocalTypeTopicCounts () {
        ...
        for (int doc = startDoc;
            ...
            for (int position = 0; position < tokens.size(); position++) {
				...
				if (topic == NewParallelTopicModel.UNASSIGNED_TOPIC) { continue; }
	...
	protected void sampleTopicsForOneDoc (FeatureSequence tokenSequence,
		...
		for (int position = 0; position < docLength; position++) {
			if (oneDocTopics[position] == NewParallelTopicModel.UNASSIGNED_TOPIC) { continue; }
		...
		for (int position = 0; position < docLength; position++) {
			...
			if (oldTopic != NewParallelTopicModel.UNASSIGNED_TOPIC) {
			...
			boolean alreadyDecremented = (oldTopic == NewParallelTopicModel.UNASSIGNED_TOPIC);
